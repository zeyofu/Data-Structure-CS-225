...
namespace cs225 {
  void PNG::_copy(PNG const & other) {
    ...
    //   if (imageData_[i]) delete imageData_[i];
    ...
    for (unsigned i = 0; i < width_ * height_; i++) {
      HSLAPixel* oth = other.imageData_[i];
      imageData_[i] = new HSLAPixel(oth->h,oth->s,oth->l,oth->a);
    }
  }
  ...
  void PNG::paste(int startX, int startY, int resolution, PNG* resized) {
      ...
      for (int x = 0; x < resolution; x++) {
        for (int y = 0; y < resolution; y++) {
          *(getPixel(startX + x, startY + y)) = *(resized->getPixel(x , y));
        }
      }
  }
  ...
  PNG::PNG(unsigned int width, unsigned int height) {
    ...
    height_ = height;
    imageData_ = new HSLAPixel*[width_ * height_];
    for (unsigned i = 0; i < width_ * height_; i++) imageData_[i] = new HSLAPixel();
  }
  ...
  bool PNG::readFromFile(string const & fileName) {
    ...
    if(imageData_) _delete();
    imageData_ = new HSLAPixel*[width_ * height_];
    for (unsigned i = 0; i < width_ * height_; i++) {
      // if (imageData_[i]) delete imageData_[i];
      imageData_[i] = new HSLAPixel();
    }
  ...
  void PNG::resize(unsigned int newWidth, unsigned int newHeight) {
    ...
    for (unsigned x = 0; x < newWidth; x++) {
      for (unsigned y = 0; y < newHeight; y++) {
        imageData_[x + (y * newWidth)] = new HSLAPixel();
